...
public static class GraphicsUtils
{
        ...
        string itemFilePath, int width, int height)
    {
        ...
        return bytes == null ? null : ByteArrayToImage(bytes);
    }
    ...
    private static string? GetTempVideoThumbnailFileName(string originalFilePath, string tempThumbnailFolder)
    {
        ...
        return string.IsNullOrEmpty(origFileName)
            ? null
            : Path.Combine(tempThumbnailFolder, Path.ChangeExtension(origFileName, ".png"));
    }
    ...
    private static bool IsWebPFormat(string path) =>
        !string.IsNullOrWhiteSpace(path) && path.EndsWith(".webp", StringComparison.OrdinalIgnoreCase);
